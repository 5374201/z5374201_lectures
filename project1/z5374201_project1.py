...
def get_tics(pth):
    ...
    with open(pth, mode='r') as fobj:
        dict = {}
        for line in fobj:
            exchange, tic = tuple(line.replace('\"', '').strip().lower().split('='))
            dict[tic] = exchange
        return dict
...
def line_to_dict(line):
    ...
    """
    # <COMPLETE THIS PART>
    dict = {column: line[i:i + COLWIDTHS[column]] for i, column in enumerate(COLUMNS)}
    return dict
    ...
    if col_lst is None:
        ...
        for i in col_lst:
            if i in COLUMNS:
                ...
                raise Exception(f"{i} contains a column that is not found in `COLUMNS`.")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    """
    # <COMPLETE THIS PART>
    #Step 1: check if tickers_lst or col_lst contains any involid elements
    ...
    for ticker in tickers_lst:
        if ticker in tic_exchange_dic:
            data_list = tic_exchange_dic[ticker]['data']
            data_dict[ticker] = []

            for data in data_list:
                data_dict[ticker].append({col: data[col] for col in col_lst})
